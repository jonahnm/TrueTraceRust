...
inline float2 AlignUV(float2 BaseUV, float4 TexScale, int2 TexDim2, float Rotation = 0) {
	...
	BaseUV = select(BaseUV < 0, (1.F - fmod(abs(BaseUV), 1.F)), fmod(abs(BaseUV), 1.F));
...
uint octahedral_32(float3 nor) {
	...
	nor.xy = (nor.xy + select(nor.xy > 0.F, t, -t)) * oct;
...
float3 i_octahedral_32( uint data ) {
    ...
    nor.xy += select((nor.xy >= 0.), -t, t);             // implementation of this
...
inline uint GenHash(float3 Pos, float3 Norm) {
	Pos = select(abs(Pos) < 0.00001f, 0.00001f, Pos);
